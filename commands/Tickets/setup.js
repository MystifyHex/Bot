module.exports.run = async (client, message, args, utils, data) => {
    ...
    const randomy = (getRandomString(4) + '-' + getRandomString(4) + '-' + getRandomString(4) + '-' + getRandomString(4))
    const randomy1 = (getRandomString(4) + '-' + getRandomString(4) + '-' + getRandomString(4) + '-' + getRandomString(4))
    const randomy2 = (getRandomString(4) + '-' + getRandomString(4) + '-' + getRandomString(4) + '-' + getRandomString(4))
    const randomy3 = (getRandomString(4) + '-' + getRandomString(4) + '-' + getRandomString(4) + '-' + getRandomString(4))
    ...
    const emojiButton = new Discord.MessageEmbed()
        ...
        .setDescription('Please send the button emoji (optional, type `no` to skip).')
        .setFooter('Type stop to cancel')
        .setColor('RANDOM');

    const buttonColor = new Discord.MessageEmbed()
        .setTitle('Ticket SetUp - 6')
        .setDescription('Please send the button color (`red`, `gray`, `blurple`, `green`).')
        .setFooter('Type stop to cancel')
        .setColor('RANDOM');
    const modRole = new Discord.MessageEmbed()
        ...
        .setDescription('Please send the tickets category (id) [optional, type `no` to skip].')
        .setFooter('Type stop to cancel')
        .setColor('RANDOM');
    const channel = new Discord.MessageEmbed()
        .setTitle('Ticket SetUp - 10')
        .setDescription('Channel, you can use `#channel` or `CHANNEL_ID`.')
        .setFooter('Type stop to cancel')
        .setColor('RANDOM');
    let d = tickets.findOne({ GuildID: message.guild.id })
    if (d.GuildID) return message.reply('This server already has a ticket system!');
        ...
        if (step == 1) {
            ...
            if (['no', 'false', 'skip'].includes(stuff.content)) {
                ...
                if (custom.id) {
                    ...
                    if (!parsed[0]) { collector.stop(); return message.channel.send('Invalid emoji...') }
            ...
            if (!['green', 'gray', 'red', 'blurple'].includes(stuff.content.toLowerCase())) return stuff.channel.send('Invalid color.')
            ...
            const channe = message.guild.channels.cache.get(stuff.content) || stuff.mentions.channels.first();
            if (!channe) {
                stuff.reply('Invalid channel.');
                return collector.stop()
            }
            rr.transcriptChannel = channe.id;
            return msg = await message.channel.send(category);
        }
        else if (step === 9) {
            const channe = message.guild.channels.cache.find(c => c.name == stuff.content && c.type == "category")
            if(['no', 'next', 'skip'].includes(stuff.content)) return msg = await message.channel.send(channel);
            if (!channe) {
                stuff.reply('Invalid category.');
                return collector.stop()
            }
            rr.category = channe.id;
            ...
            try {
                ...
                if (sending) {
                    sending.delete();
                    return message.channel.send('Whoopsie poopsie! Error while setting up, make sure i have permission!');
                }
                return message.channel.send('Whoopsie poopsie! Error while setting up, make sure i have permission!');
            }
            const embed2 = new Discord.MessageEmbed()
                .setDescription(`Alright ticket setted up. Click [here](https://discord.com/channels/${message.guild.id}/${rr.channel.id}/${sending.id}) to jump to it!`)
                .setColor('BLUE');
