...
module.exports = async (client, message) => {
	...
	await s.findOne({ guildID: message.guild.id }).lean().exec().then(async (data) => {
		...
		if (data.suggestionsChannel == message.channel.id) { client.suggest(message, message.content); }
		...
		const cooldown = require('ms')(client.commands.get(command).config.cooldown)
		...
		if (timestamps.has(message.author.id)) {
			...
			if (Date.now() < expirationTime) {
				...
				const timeLeft = require('ms')(expirationTime);
				return message.channel.send(new Discord.MessageEmbed().setTitle(`${message.author.username}, this command is in cooldown...`).setDescription(`This command is on cooldown for **<t:${timeLeft}>**`).setColor('RED'));
			}
		}
